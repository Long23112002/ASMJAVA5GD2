                            ...
                            <div class="card-body">
                                <form action="${pageContext.request.contextPath}/bill/update/${bill.id}" method="post"  >
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select id="name" class="form-select" name="idCustomer" aria-label="Default select example">
                                            <c:forEach items="${listCustomer}" var="customer">
                                                <option value="${customer.id}" <c:if test="${bill.idCustomer == customer.id ? 'selected' : ''}"> </c:if> >${customer.name}</option>
                                            </c:forEach>
                                        </select>
                                        <c:if test="${ not empty errors}">
                                            <span id="code-error" class="text-danger">${errors['idCustomer']}</span>
                                        </c:if>
                                    </div>
                                    ...
                                    <div class="mb-3">
                                        <select class="form-select" name="status" aria-label="Default select example">
                                            <option  value="0" <c:if test="${bill.status eq 0} ? 'selected' : '' "></c:if> >Pending</option>
                                            <option value="1"  <c:if test="${bill.status eq 1} ? 'selected' : '' "></c:if> >Completed</option>
                                            <option value="2"  <c:if test="${bill.status eq 2} ? 'selected' : '' "></c:if> >Cancel</option>
                                        </select>
